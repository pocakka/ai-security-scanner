...
function domainToSlug(domain: string): string {
  // Convert domain to URL-safe slug: openai.com -> openai-com
...
export default function ScanResultPage() {
          ...
          <div className="relative max-w-6xl mx-auto px-6 pt-20 pb-32">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-400 text-sm mb-12">
                Requested: /s/{scanNumberParam}/{domainSlug}
              </p>
  ...
  // **10/10 LAYOUT**: Separate AI-related findings from traditional security findings

  // AI-specific categories (will be shown in unified "AI Security" section)
  const aiSecurityCategories = ['owasp-llm01', 'owasp-llm02', 'owasp-llm05', 'owasp-llm06', 'owasp-llm07', 'owasp-llm08']
  const generalAiCategory = 'ai' // General AI presence finding

  // Traditional security categories
  const traditionalSecurityCategories = ['reconnaissance', 'admin', 'port', 'backend-framework', 'web-server', 'frontend-framework', 'api-security', 'client', 'ssl', 'cors', 'dns', 'cookie', 'security', 'library', 'compliance', 'waf', 'mfa', 'rate-limit', 'graphql', 'error-disclosure', 'spa-api']

  // Split findings into AI Security and Traditional Security
  const aiSecurityFindings = findings.filter((f: any) => aiSecurityCategories.includes(f.category))
  ...
  const traditionalCategoriesToShow = isFullReport
    ...
    : traditionalSecurityCategories.filter(cat => findingsByCategory[cat])
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        {/* **10/10 LAYOUT**: UNIFIED AI SECURITY SECTION */}
        {/* Show if AI Trust Score detected AI OR if there are any AI-related findings */}
        {(scan.aiTrustScorecard?.hasAiImplementation || aiSecurityFindings.length > 0 || generalAiFindings.length > 0) && (
          <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl shadow-xl p-6 mb-8">
            ...
            <div className="mb-6 pb-4 border-b border-white/20">
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1">
                  <h2 className="text-2xl font-bold text-white mb-1">Artificial Intelligence Security</h2>
                  <p className="text-sm text-slate-400 mb-2">AI technologies, vulnerabilities, and security analysis</p>
                </div>
                {(aiSecurityFindings.length > 0 || generalAiFindings.length > 0) && (
                  <div className="text-right">
                    <div className="text-3xl font-bold text-white">{aiSecurityFindings.length + generalAiFindings.length}</div>
              ...
              <div className="bg-blue-500/10 border border-blue-400/30 rounded-lg p-3">
                <p className="text-sm text-blue-200">
                  <strong>What does this mean?</strong> This section shows all AI-related security information including detected technologies, AI-specific vulnerabilities (OWASP LLM Top 10), and general AI implementation risks.
                </p>
              </div>
            </div>

            {/* SUBSECTION 1: Detected Technologies */}
            {scan.aiTrustScorecard?.hasAiImplementation && (
              <div className="mb-8">
                <div className="flex items-center gap-2 mb-4">
                  <span className="text-2xl">📊</span>
                  <h3 className="text-xl font-semibold text-white">Detected Technologies</h3>
                </div>
            ...
            {/* SUBSECTION 3: General AI Presence (Info-level finding) */}
            {generalAiFindings.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-4 border-t border-white/20 pt-6">
                  <span className="text-2xl">💡</span>
                  <h3 className="text-xl font-semibold text-white">General AI Presence</h3>
                </div>
                <div className="space-y-4">
                  {generalAiFindings.map((finding: any, index: number) => (
                    <FindingCard key={`general-ai-${index}`} finding={finding} knowledgeBase={knowledgeBase} />
                  ))}
                </div>
              </div>
            )}
        ...
        {/* **10/10 LAYOUT**: Traditional Security Findings Section (non-AI) */}
        <div className="space-y-6">
          {traditionalCategoriesToShow.length === 0 ? (
            <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl shadow-xl p-12 text-center">
              ...
              <p className="text-slate-300 text-lg">No traditional security issues found on this website.</p>
            </div>
          ) : (
            <>
              {/* Section Title for Traditional Security */}
              <div className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-white/20 rounded-2xl shadow-xl p-6 mb-6">
                <h2 className="text-2xl font-bold text-white mb-2">Traditional Security Findings</h2>
                <p className="text-sm text-slate-400">Standard web security issues unrelated to AI implementation</p>
              </div>

              {traditionalCategoriesToShow.map((category: string) => {
                const categoryFindings = findingsByCategory[category] || []
